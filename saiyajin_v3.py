...
def process_type(args):
    type, year = args
    try:
        data = pd.read_csv(f"URLS/urls_{year}_{type}.csv")
    except Exception as e:
        print(f"Error reading CSV for {type} in {year}: {e}")
        return

    start_time = time.time()
    try:
        out = get_output_data(data)
        output_name = f"output_{year}_{type}.csv"
        if not os.path.exists('DATA'):
            os.makedirs('DATA')
        out.to_csv(f"DATA/{output_name}", index=False)
        print(f"Save data type {type} from year {year} as: {output_name}")
    except Exception as e:
        print(f"Error processing data for {type} in {year}: {e}")
    end_time = time.time()
    execution_time1 = -(start_time - end_time) / 60
    print(f"Execution time to analyze images from urls: {execution_time1} minutes")
    print("Done...")
...
if __name__ == '__main__':
    ...
    subprocess.run([sys.executable, "api_actualizador.py"])
    
    subprocess.run(["bash", "commit.sh"])
