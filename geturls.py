    ...
    try:
        ...
        for link in soup2.find_all('a')[5:]:
            if "_1024." in link.get('href'):
                full_url = f"{url}{link.get('href')}"
                urls2.append(full_url)
        ...
        print(f"[ERROR] Falló la solicitud para {year}/{type}/{m}: {e}")
        return [], [], [], [], []


def get_all_files(year, type):
    try:
        url = f"{path}{year}/{type}"
        response = requests.get(url, headers=HEADERS, timeout=10)
        response.raise_for_status()
        ...
        urls_data["date"] = pd.to_datetime(urls_data[['Year', 'Month', 'Day', 'Hour', 'Minute']])
        urls_data = urls_data.drop(columns=['Year', 'Month', 'Day', 'Hour', 'Minute'])
        urls_data = urls_data[["date", "url"]]
        return urls_data

    except requests.exceptions.RequestException as e:
        print(f"[ERROR] Falló la solicitud principal para {year}/{type}: {e}")
